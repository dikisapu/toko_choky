...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $sql_get_last_id = "SELECT barang_id FROM barang ORDER BY barang_id DESC LIMIT 1";
    $result = $conn->query($sql_get_last_id);
    $last_id = 0;
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $last_id = (int)substr($row['barang_id'], 1);
    }
    $new_id = 'B' . str_pad($last_id + 1, 3, '0', STR_PAD_LEFT);

    $nama_barang = $_POST['nama_barang'];
    $harga_barang = $_POST['harga_barang']; // Tambahkan baris ini untuk mendapatkan harga_barang dari form

    $sql = "INSERT INTO barang (barang_id, nama_barang, harga_barang)
            VALUES ('$new_id', '$nama_barang', '$harga_barang')"; // Tambahkan harga_barang ke dalam perintah SQL
                                ...
                                <div class="card-body">
                                    ...
                                    <form action="tambah_barang.php" method="post">
                                        <div class="form-group">
                                            ...
                                            <input type="text" id="barang_id" name="barang_id" value="<?php echo $new_id; ?>" class="form-control" readonly>
                                        </div>
